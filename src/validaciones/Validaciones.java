...
public class Validaciones {
    public static int validarEdad(String text) {
        ...
        try {
            ...
            if(idade<0 | idade>125)throw new NumberFormatException("La edad introducida no es valida recuerde introduzca un numero entre 0 y 125") ;
            else edad=idade;
        } catch (EdadFormatException e){
            JOptionPane.showMessageDialog(null, e);
        } catch (NumberFormatException e){
            JOptionPane.showMessageDialog(null, "Edad no Válida: recuerde debe ser valores nuericos");
        }
        return edad;
    }
    ...
    public static String validarNombre(String text) {
        String nombre="";
        try{
            if(text.trim().length()>0 && text.length()<50)nombre=text;
